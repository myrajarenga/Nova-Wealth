...
import { getBookmarks, toggleBookmark } from '../../lib/resourcesApi.js'
  ...
  return (
    <article className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <p className="text-sm text-[#2C3E50]">{r.summary}</p>
        ...
        <div className="mt-4 flex items-center gap-2">
          <button disabled={locked} onClick={() => track('resource_download', { id: r.id })} className={`px-3 py-2 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 ${locked ? 'bg-gray-200 text-gray-500' : 'bg-navy text-white hover:bg-black focus:ring-navy'}`}>{locked ? 'Client Only' : 'Download'}</button>
          <button disabled={locked} onClick={() => track('resource_card_view', { id: r.id })} className={`px-3 py-2 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 ${locked ? 'bg-gray-200 text-gray-500' : 'bg-gold text-black hover:bg-[#c39b32] focus:ring-gold'}`}>{locked ? 'Login Required' : 'View'}</button>
